...
router.get("/myFollowingPost", requiredLogin, (req, res) => {
    ...
    Post.find({ postedBy: { $in: req.user.followings } }).populate("postedBy", "_id name")
        .populate("comments.postedBy", "_id name")
        .sort("-_id")
        .then(posts => {
            res.json(posts);
...
router.get("/myPost", requiredLogin, (req, res) => {
    Post.find({ postedBy: req.user._id })
        .populate("postedBy", "_id name email").then(myPosts => {
...
router.put('/deleteComment/:postId', requiredLogin, (req, res) => {
    Post.findByIdAndUpdate(req.params.postId, { $pull: { comments: { postedBy: req.user._id, text: req.body.text } } }, { new: true })
        .populate("postedBy", "_id")
        .populate("comments.postedBy", "_id name")
        .exec((err, result) => {
            if (err) {
