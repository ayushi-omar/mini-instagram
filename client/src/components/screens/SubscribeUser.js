            ...
            {data.map(item => {
                return (
                    <div className="card home-card" key={item._id}>
                        {item.postedBy && item.postedBy._id.toString() === state._id.toString() &&
                            <i className="material-icons" style={{ float: "right" }} onClick={() => { deletePost(item._id) }}>delete</i>
                        }
                        <h5 style={{ padding: "10px" }}><Link to={item.postedBy && item.postedBy._id.toString() !== state._id.toString() ? `/profile/${item.postedBy._id}` : `/profile`}>{item.postedBy.name}</Link></h5>
